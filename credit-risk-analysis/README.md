...
```text
credit-risk-analysis/
├── credit-risk-modeling.R
├── credit-risk-data.txt
└── README.md
```

`credit-risk-modeling.R` contains the complete analysis and modeling workflow, while `credit-risk-data.txt` contains the dataset used in the study (the German Credit dataset format, one record per line).
